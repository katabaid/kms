...
def get_mcu(price_list: str) -> list[dict]:
	return frappe.db.sql("""
		...
		WHERE
			...
			AND tip.price_list = %s""", (price_list), as_dict=True)
...
def _check_appointment_eligibility(name: str) -> list[dict]:
	return frappe.db.sql("""
		...
		FROM appointment_checks""", {"appt": name}, as_dict=True)
...
def get_latest_item_price(item_code, price_list):
	...
	price = frappe.db.sql("""
		...
		LIMIT 1""", (item_code, price_list, today_date, today_date), as_dict=True)
